...
//   for( int k=0;k < vecsize; ++k){
 //  	            double point[3]; point[0]=pointi.x[k];point[1]=pointi.y[k];point[2]=pointi.z[k];
   ...
   //      }     
    ...
    //  TGeoTube::DistFromInsideS_v(pointi, diri, this->TGeoTube::GetRmin(), this->TGeoTube::GetRmax(), this->TGeoTube::GetDz(), dist, vecsize );
 //  }
...
// REAL VECTOR INTERFACES: THIS IMPLEMENTATION SHOULD BE GRADUALLY MOVED SOMEWHERE ELSE 
   ...
   virtual void DistFromOutsideSOA_v( StructOfCoord  const & pointi, StructOfCoord  const & diri, Int_t /*iact*/, Double_t const * step, Double_t * /*safe*/ , Double_t * dist, Int_t vecsize ) const {
      //  TGeoTube::DistFromOutsideS_v(pointi, diri, this->TGeoTube::GetRmin(), this->TGeoTube::GetRmax(), this->TGeoTube::GetDz(), step, dist, vecsize );
        TGeoTube::DistFromOutsideS_v(pointi, diri, this->TGeoTube::GetRmin(), this->TGeoTube::GetRmax(), this->TGeoTube::GetDz(), dist, vecsize );
}

static void DistFromOutsideS_v(StructOfCoord const & pointi, StructOfCoord const & diri, Double_t rmin, Double_t rmax, Double_t dz, Double_t * distance, Int_t np);

static void DistFromInsideS_v(StructOfCoord const & pointi, StructOfCoord const & diri, Double_t rmin, Double_t rmax, Double_t dz, Double_t * distance, Int_t np);
